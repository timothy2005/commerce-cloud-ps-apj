 ...
 */
function iterateAndCompareSignatures(
    oldSignatureObject,
    newSignatureObject,
    attributeWithSignatures
) {
    const oldArgumentsOrResult = removeDuplicatesAndMerge(
        oldSignatureObject.list,
        attributeWithSignatures
    );

    const newArgumentsOrResult = removeDuplicatesAndMerge(
        newSignatureObject.list,
        attributeWithSignatures
    );

    const signaturesDiffElements = getSignaturesDiffElements(
        oldArgumentsOrResult,
        newArgumentsOrResult
    );

    if (signaturesDiffElements.length) {
        return (
            oldSignatureObject.serviceName +
            ', ' +
            oldSignatureObject.functionName +
            '\n' +
            (oldSignatureObject.arguments
                ? 'arguments: ' + JSON.stringify(oldSignatureObject.arguments) + '\n'
                : '') +
...
module.exports = {
    execute: function(oldVersion, newVersion) {
        const OLD_VERSION_FILE_NAME = bundlePaths.report.instrument_functions_file.replace(
            'VERSION',
            oldVersion
        );
        const NEW_VERSION_FILE_NAME = bundlePaths.report.instrument_functions_file.replace(
            'VERSION',
            newVersion
        );

        prepareDataObject(NEW_VERSION_FILE_NAME, ['serviceName', 'functionName'], function(
            newVersionObject
        ) {
            prepareDataObject(OLD_VERSION_FILE_NAME, ['serviceName', 'functionName'], function(
                oldVersionObject
            ) {
                ...
                Object.keys(oldVersionObject).forEach((key) => {
                    ...
                    if (newSignatureObject === undefined) {
                        ...
                        diffArgumentsData += iterateAndCompareSignatures(
                            oldSignatureObject,
                            newSignatureObject,
                            'arguments'
                        );
                ...
                const backwardCompatibilityResults =
                    bundlePaths.report.backwardCompatibilityResults;
                fs.mkdirs(backwardCompatibilityResults);
                saveResult(
                    `${backwardCompatibilityResults}/${newVersion}/functionsFrom${oldVersion}NotInNewVersion.data`,
                    'Following requests from previous version were not found in new version',
                    notInNewVersionData
                );
                saveResult(
                    `${backwardCompatibilityResults}/${newVersion}/functionsArgumentsDiffWith${oldVersion}.data`,
                    'Following requests have different arguments between both versions',
                    diffArgumentsData
                );
            });
        });
                ...
                prepareDataObject(
                    ...
                    function(oldVersionObject) {
                        ...
                        Object.keys(oldVersionObject).forEach((key) => {
                            ...
                            const newSignatureObject = newVersionObject[key];
                            if (newSignatureObject !== undefined) {
                                diffResultData += iterateAndCompareSignatures(
                                    oldSignatureObject,
                                    newSignatureObject,
                                    'result'
                                );
                        ...
                        fs.mkdirs(backwardCompatibilityResults);
                        saveResult(
                            `${backwardCompatibilityResults}/${newVersion}/functionsReturnsDiffWith${oldVersion}.data`,
                            'Following requests have different results from a version to another',
                            diffResultData
                        );
                    }
                );
