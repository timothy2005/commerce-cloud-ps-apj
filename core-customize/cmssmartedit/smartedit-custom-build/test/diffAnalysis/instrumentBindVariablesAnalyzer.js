...
module.exports = {
    execute: function(oldVersion, newVersion) {
        const OLD_VERSION_FILE_NAME = bundlePaths.report.instrument_directives_file.replace(
            'VERSION',
            oldVersion
        );
        const NEW_VERSION_FILE_NAME = bundlePaths.report.instrument_directives_file.replace(
            'VERSION',
            newVersion
        );
        ...
        prepareDataObject(NEW_VERSION_FILE_NAME, function(newVersionObject) {
            prepareDataObject(OLD_VERSION_FILE_NAME, function(oldVersionObject) {
                ...
                Object.keys(oldVersionObject).forEach((componentName) => {
                    ...
                    if (newObj === undefined) {
                        notInNewVersionData +=
                            componentName + ', ' + JSON.stringify(oldObj) + '\n\n';
                    } else {
                ...
                saveResult(
                    `${backwardCompatibilityResults}/${newVersion}/componentsFrom${oldVersion}NotInNewVersion.data`,
                    'Following components from previous version were not found in new version',
                    notInNewVersionData
                );
                saveResult(
                    `${backwardCompatibilityResults}/${newVersion}/componentsBindingDiffWith${oldVersion}.data`,
                    'Following components have diff bind variables between both versions',
                    diffBindVariables
                );
            });
        });
