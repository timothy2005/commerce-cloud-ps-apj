...
export class YSelectComponent<T extends { id: string }> {
    ...
    public resultsHeaderLabel = 'se.yselect.options.inactiveoption.label';
    ...
    $onInit(): void {
        ...
        this.exposedModel.$render = (): void => this.syncModels();

        this.reset = (forceReset: boolean): Promise<any> => {
            this.items.length = 0;
    ...
    public refreshOptions(mask: string): void {
        if (this.fetchStrategy.fetchAll) {
            (this.fetchStrategy.fetchAll(mask) as Promise<T[]>).then((items: T[]) => {
                if (angular.toJson(this.items) !== angular.toJson(items)) {
                    this.items = items;
                }
            });
        }
    ...
    public internalFetchAll(): Promise<any> {
    ...
    public updateModelIfNecessary(): void {
    ...
    private _updateChild(): void {
        ...
        this.result = (rawTemplate as string)
            .replace('<%= theme %>', theme)
