...
describe('ySelectModule - ', () => {
    ...
    describe('Validation', () => {
        ...
        it('will throw validation errors if component is provided with fetchPage but not with fetchEntity or fetchEntities', () => {
            ...
            expect(() => controller.$onChanges()).toThrow(
                new Error(
                    `fetchPage has been specified in fetchStrategy but neither fetchEntity nor fetchEntities are available to load item identified by ${testScopeSingle.model}`
                )
            );
        });
    });
